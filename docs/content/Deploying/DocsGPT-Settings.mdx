...
- **`SERVE_UI`**: Whether the API process serves the web UI shipped in the Python package (`pip install docsgpt`). Defaults to `true`; set `false` when a separate frontend, such as the Docker frontend image, serves the UI. Has no effect in the backend Docker image, which carries no UI build.
...
OpenScout intelligence JSON requests and report exports have independent limits
so large comparison or report payloads cannot consume unbounded memory, database,
LLM, or PDF-rendering resources.

| Setting | Default | Description |
| --- | --- | --- |
| `INTELLIGENCE_MAX_JSON_BYTES` | `2097152` | Maximum JSON request body for `/api/intelligence/*`; oversized requests return HTTP 413. |
| `INTELLIGENCE_MAX_REPORT_CHARS` | `2000000` | Maximum serialized report size accepted by Markdown and PDF export. |

These settings can be overridden in `.env`. Structured request models also cap
project selections, comparison dimensions, result collections, evidence, claims,
sections, sources, and their text fields.
...
`CORS_ALLOWED_ORIGINS` is a comma-separated list of exact browser origins. When
unset, the backend allows only the local frontend origins
`http://localhost:5173`, `http://localhost:5174`,
`http://127.0.0.1:5173`, and `http://127.0.0.1:5174`. Wildcards are not
accepted. In no-auth mode, configured origins are still restricted to loopback;
use `simple_jwt`, `session_jwt`, or `oidc` before allowing a non-loopback origin.
Requests without an `Origin` header remain valid local API calls, while unknown
...
| Value         | Description                                                                                 |
| ------------- | ------------------------------------------------------------------------------------------- |
| `None`        | No authentication is used. Only loopback clients can access the app.                       |
