...
from docsgpt.storage.db.embeddings_pin import (  # noqa: E402
    resolve_embeddings_pin,
    warn_on_source_model_mismatch,
)
from docsgpt.stt.upload_limits import (  # noqa: E402
    build_stt_file_size_limit_message,
    should_reject_stt_request,
)
from docsgpt.upload_limits import (  # noqa: E402
...
ensure_database_ready(
    ...
    logger=logging.getLogger("docsgpt.app"),
)
...
resolve_embeddings_pin(logging.getLogger("docsgpt.app"))
warn_on_source_model_mismatch(logging.getLogger("docsgpt.app"))
...
if settings.AUTO_VECTOR_SCHEMA:
    _vector_schema_log = logging.getLogger("docsgpt.app")
    try:
...
# if docsgpt.app is re-imported (coverage tests reload the module): without
...
def authenticate_request():
    ...
    # auth to the route handlers (see docsgpt/api/v1/routes.py).
